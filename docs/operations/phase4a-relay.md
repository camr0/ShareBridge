...
error, `3` PARTIAL (no failures but at least one SKIP, or a dry run). Note: the
sibling M4-exit harness (`§14`) additionally exits `2` for an **incomplete**
`--dry-run` configuration after naming every missing variable; this harness does
no config validation in `--dry-run` and therefore always exits `3` there.
...
| `LIVE_PHASE4A_RELAY_HOST` | — | relay VM SSH target (topology, firewall, health, ordering); **also** the direct SSH target for the loopback `/metrics` read when `LIVE_PHASE4A_GATEWAY_METRICS_URL` is unset, so it must be user-qualified when the harness host has no default user for that machine (`root@10.0.0.5`, not a bare IP) |
...
| `LIVE_PHASE4A_FRPS_RESTART_CONFIRM` | — | `acceptance_09`: the EXACT `<ssh-host>|<systemd-unit>` the harness may restart (must equal `LIVE_PHASE4A_FRPS_SSH_HOST`\|`LIVE_PHASE4A_FRPS_UNIT`); refuses the restart when unset or mismatched |
| `LIVE_PHASE4A_AGENT_SSH_HOST` | — | `acceptance_09` frpc child identity (unset = the harness host runs the agent) |
| `LIVE_PHASE4A_FRPC_PID_MATCH` | `frpc` | `acceptance_09` `pgrep -f` **pre-filter** for the child; the identity is restricted to the exact process name so wrapper command lines never count |
| `LIVE_PHASE4A_FRPC_PID_NAME` | `frpc` | `acceptance_09` exact process name (comm) of the frpc child for the default identity discovery |
| `LIVE_PHASE4A_FRPC_IDENTITY_CMD` | — | `acceptance_09` operator override: a command on the agent host whose stdout is exactly one `PID:STARTTIME` identity (for containerised agents) |
| `LIVE_PHASE4A_FRPS_JOURNAL_FILE` | — | `acceptance_09` operator-supplied frps journal (else `journalctl -u LIVE_PHASE4A_FRPS_UNIT` over SSH) for the agent-specific fresh-session proof |
| `LIVE_PHASE4A_JOURNAL_MAX_LINES` | `5000` | `acceptance_09` journal tail line budget for SSH reads |
| `LIVE_PHASE4A_AGENT_PROXY_NAME` | `sb-<LIVE_PHASE4A_NAMESPACE>` | `acceptance_09` frps proxy name for the agent-specific `new proxy [<name>] type [tcp] success` proof |
| `LIVE_PHASE4A_RECOVERY_BOUND_S` | `120` | `acceptance_09` automatic-recovery bound (measured after all stages; each stage capped by the remaining time) |
...
| `acceptance_09_restart_recovery` | 43 | §19 #9: baseline tunnel online + the configured share serves over the relay; the restart target is exactly confirmed (`LIVE_PHASE4A_FRPS_RESTART_CONFIRM`); restart the pinned frps unit over SSH with the agent's frpc child ALIVE (identity re-measured right after the restart); observe `online=0`; then, with **no operator action**, require a REPLACED `PID:STARTTIME` child, a NEW tunnel session, a NEW frps `new proxy [<proxy>] type [tcp] success` line **for the configured agent proxy name** (the global reconnect counter is corroboration only), gateway `online=1` and serving relay content, with the elapsed seconds measured after every stage and required to be within `LIVE_PHASE4A_RECOVERY_BOUND_S`. Opt-in `LIVE_PHASE4A_ALLOW_RESTART=1`; fails closed with a named reason when any stage (including an ambiguous 0/>1 child identity, an unconfirmed restart target, or an unobservable agent-specific session signal) cannot be proven | control base URL + share code, gateway metrics, frps/agent SSH targets, frps journal source, exact restart-target confirmation |
...
SKIP, or a dry run — a run that executes nothing is never a pass). This differs
deliberately from `§13` (`scripts/live-phase4a.sh`), which always exits `3` for
`--dry-run`: here an unset required input is an operator-usage error and exits
`2` after naming every missing variable, so a wrapper can tell "fix your env"
from "a dry run".
...
`LIVE_M4EXIT_CONTROL_BASE_URL` — the operator has to type the exact target to
assert it is the disposable test stack and not production or the M6 dark
topology. The lockdown case's auto-unlock safety net is installed in the
**main** process (`EXIT`/`INT`/`TERM`), not inside the case subshell (where a
`local` flag was unreachable under `set -u`); the locked state is kept in a
state file so a Ctrl-C, a `SIGTERM` to the harness PID, or any failing exit
re-sends `POST /api/unlock`. The state is written **before** the lockdown
request is issued and cleared only after a definitively unapplied request or a
successful unlock. `--selftest` proves the trap (including the
mark-before-request window) with a stubbed admin API and nothing live.

Hygiene contract (stated accurately, not overstated): configured secrets (admin
password, share codes) and token/key/password/secret/cookie/authorization/`jti`
values — including JSON `"key": "value"` pairs — are redacted before they reach
the console, the evidence files or the captured command/output records;
private-key blocks and `/s/<code>` paths are redacted too. Redaction is
BEST-EFFORT pattern matching (single-line JSON; multi-line pretty-printed JSON
still relies on the text patterns), not a guarantee, so treat evidence as
operator-internal. Raw HTTP capture bodies and headers live only in one per-run
0700 scratch directory under `umask 077` and are deleted by the
`EXIT`/`INT`/`TERM` cleanup hook (also on error). The default evidence root is
`${TMPDIR:-/tmp}/sharebridge-m4exit-e2e`, so a run never dirties the worktree.
...
| `stun_observe_and_rechallenge` | control `stun_total{match}>0` with `mismatch=0` and `timeout=0`; the `§10.2` proactive rechallenge cadence (4m + jitter[0,15s), tolerance configurable) measured from the `stun observation accepted` timestamps **of the agent under test only**, with the newest acceptance proven **fresh** (≤ the 4m+jitter ceiling + tolerance) so a stale journal cannot pass | control metrics access, STUN journal source, `LIVE_M4EXIT_STUN_AGENT_ID` |
| `direct_path_or_failclosed` | either `status=direct` with a direct URL that serves (`200`), or a fail-closed relay fallback whose control-side agent record shows `direct_status=relay_fallback` and the expected `direct_status_reason` **and is provably current** (updated at/after the prepare-route request and tied to `LIVE_M4EXIT_AGENT_RECORD_AGENT_ID`); FAILs when neither is observable. The record is obtained either by the harness running `LIVE_M4EXIT_AGENT_RECORD_COMMAND` (its stdout is the record; preferred, because it runs after `prepare-route` and so is naturally current) or from an operator-supplied `LIVE_M4EXIT_AGENT_RECORD_FILE` exported after the request. Expected record shape (control PocketBase `agents` collection, JSON or key=value): `direct_status`, `direct_status_reason`, `api_key_id` (or the record id), and a timestamp in `updated`/`stun_observed_at`/`relay_last_seen_at` (`YYYY-MM-DD HH:MM:SS`). A stale timestamp or a foreign agent id is always refused | control base URL, share code, agent record command **or** file + agent id (for the relay case) |
| `lockdown_withdrawal_and_recovery` (**opt-in**) | baseline healthy **and the relay URL actually serves (200 with content)** → lockdown (conservative "may be locked" state marked before the request) → tunnel `online=0` within the bound + a frps `proxy closing` line + `prepare-route` `503` suppressed + a post-lock relay fetch proving the real withdrawal property: **no HTTP 2xx, no baseline body (exact, prefix, same length or content marker), and no non-2xx body larger than the documented small maximum (`LIVE_M4EXIT_WITHDRAWAL_MAX_ARTIFACT_BYTES`, default 64)** — a small TLS-level teardown artifact (observed live as 30 bytes with curl exit 35) is tolerated, but a truncated/slightly-modified copy of the real content of any size is not → unlock → tunnel online + `prepare-route` relay again within the bound (monotonic: measured after every stage, each capped by the remaining time), with the measured recovery seconds recorded. Refuses to lock down if the baseline does not serve | `ALLOW_LOCKDOWN=1`, `TARGET_CONFIRM`, agent admin, gateway metrics, frps journal |
...
| `tunnel_recovery_frps_restart` (**opt-in**) | the release-blocking dropped-session scenario: baseline tunnel `online=1` **and** the configured share actually serves over the relay → the exact restart target (`LIVE_M4EXIT_FRPS_RESTART_CONFIRM`) is confirmed and the pinned frps unit is restarted while the agent's frpc child stays ALIVE (the identity is re-measured right after the restart and must still be the old `PID:STARTTIME` child) → tunnel `online=0` observed within the bound → then, with **no operator action**, a REPLACED child, a NEW tunnel session, a NEW frps `new proxy [<proxy>] type [tcp] success` line **for this agent's proxy** (`LIVE_M4EXIT_AGENT_PROXY_NAME`, default `sb-<namespace>`; the global reconnect counter is corroboration only), `online=1` and serving relay content, with the elapsed seconds measured after every stage and required to be within the bound. Ambiguous child identities, an unconfirmed restart target and an unobservable agent-specific session signal all fail closed. The harness never restarts the agent or its child | `ALLOW_FRPS_RESTART=1`, `FRPS_RESTART_CONFIRM`, `TARGET_CONFIRM`, control base URL + share code, gateway metrics, frps SSH target + journal |
...
| `LIVE_M4EXIT_AGENT_RECORD_FILE` / `LIVE_M4EXIT_AGENT_RECORD_COMMAND` / `LIVE_M4EXIT_AGENT_RECORD_AGENT_ID` / `LIVE_M4EXIT_EXPECTED_DIRECT_REASON` / `LIVE_M4EXIT_AGENT_RECORD_FRESHNESS_TOLERANCE_S` | — / — / — / `probe_failed` / `0` | case 4 relay fallback. The command is preferred: the harness runs it after `prepare-route` (stdout = the record), so its timestamp can postdate the request. The source must parse as **exactly ONE** structured record (a multi-record JSON list, an `{"items":[...]}` list with >1 item, or a key=value dump repeating an identity/timestamp key is refused as ambiguous); the agent id must be an **exact `api_key_id`/`id` field**; the chosen timestamp (`updated` first, then `stun_observed_at`, then `relay_last_seen_at`) must not predate the request floor (default tolerance `0` = strict). `direct_status`/`direct_status_reason` are read from that same record. A stale record, a foreign agent id or an ambiguous dump is refused |
...
| `LIVE_M4EXIT_ALLOW_FRPS_RESTART` / `_TUNNEL_RECOVERY_BOUND_S` / `_FRPC_PID_MATCH` / `_FRPC_PID_NAME` / `_FRPC_IDENTITY_CMD` / `_AGENT_PROXY_NAME` / `_FRPS_RESTART_CONFIRM` / `_WITHDRAWAL_MAX_ARTIFACT_BYTES` | `0` / `120` / `frpc` / `frpc` / — / `sb-<relay-URL namespace>` / — / `64` | case 7 (`tunnel_recovery_frps_restart`; opt-in frps restart). `FRPS_RESTART_CONFIRM` must equal `<LIVE_M4EXIT_FRPS_SSH_HOST>`\|<LIVE_M4EXIT_FRPS_UNIT>; `FRPC_PID_MATCH` is only the `pgrep -f` pre-filter; `WITHDRAWAL_MAX_ARTIFACT_BYTES` bounds the tolerated non-2xx teardown artifact while locked (case 5) |
