...
| `LIVE_M4EXIT_AGENT_RECORD_FILE` / `LIVE_M4EXIT_AGENT_RECORD_COMMAND` / `LIVE_M4EXIT_AGENT_RECORD_AGENT_ID` / `LIVE_M4EXIT_EXPECTED_DIRECT_REASON` | — / — / — / `probe_failed` | case 4 relay fallback. The command is preferred: the harness runs it after `prepare-route` (stdout = the record), so its `updated` timestamp can postdate the request; the file must be exported after the request. The record must carry the agent id and a timestamp (`updated`/`stun_observed_at`/`relay_last_seen_at`, `YYYY-MM-DD HH:MM:SS`); a stale record or a foreign agent id is refused |
