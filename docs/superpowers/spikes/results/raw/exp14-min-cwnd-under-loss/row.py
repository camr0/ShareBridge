#!/usr/bin/env python3
"""E14 row extractor: like the E5 one plus min_cwnd and the wire/payload ratio.

usage: row.py <label> <json_path> <log_path>

Columns (tab separated):
label mbps ceil_mbps min_cwnd wire_ratio dg_per_mib rtt loss wall_s nstall stall_s longest_s drop werr fwd error
...
def main():
    ...
    if j is None:
        ...
        res_mbps = mcwnd = rtt = loss = dgmib = ratio = ""
    else:
        err = (j.get("error") or "").replace("\t", " ")
        res_mbps = "%.2f" % j.get("mbps", 0)
        mcwnd = j.get("min_cwnd", "")
        rtt = j.get("rtt_ms", "")
        loss = j.get("loss", "")
    ...
    print("\t".join(str(x) for x in [
        label, res_mbps, "%.1f" % ceil, mcwnd, ratio, dgmib, rtt, loss, "%.1f" % wall,
