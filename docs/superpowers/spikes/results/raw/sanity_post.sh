...
for rep in 1 2 3 4 5; do
  ...
  echo "{\"tag\":\"$tag\",\"status\":\"$([ $rc -eq 0 ] && echo ok || echo error)\",\"rc\":$rc,\"wall_s\":$t1,\"load0\":\"$load0\",\"result\":$(cat "$out" 2>/dev/null || echo null)}" >> "$OUTDIR/sanity_post.jsonl"
done
