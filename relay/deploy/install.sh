...
#              [--gateway-binary <file>] [--frps-binary <file>] [--dry-run]
...
# audit_dns <namespace> <tunnel-host> [relay-ipv4] — prove the §6 DNS
# invariant: the relay wildcard and the tunnel host are DNS-only (A/AAAA) and
# the zone publishes no HTTPS/SVCB records, so no browser can ever obtain an
# ECH key that would hide the SNI the gateway routes on.
audit_dns() {
  local namespace="$1" tunnel_host="$2" expected_ipv4="${3:-}"
  local zone="sharebridgeusercontent.com"
  local relay_wildcard="*.relay.${namespace}.${zone}"
  local status=0

  log "DNS audit: ${relay_wildcard} and ${tunnel_host}"
  log "expected relay IPv4: ${expected_ipv4:-<not given>}"

  local relay_a tunnel_a
  ...
  # An HTTPS or SVCB RR (in particular one carrying an ech= parameter) hides
  # SNI from the gateway and the agent Binder and breaks exact routing.
  local https_relay https_tunnel svcb_relay ech_probe
  ...
  ech_probe="$(dig +short HTTPS "${relay_wildcard}" "${tunnel_host}" 2>/dev/null | grep -i 'ech=' || true)"
  if [[ -n "$https_relay" || -n "$https_tunnel" || -n "$svcb_relay" || -n "$ech_probe" ]]; then
    log "FAIL: zone publishes HTTPS/SVCB (ECH-capable) records for relay origins"
    log "  HTTPS ${relay_wildcard} -> ${https_relay:-<none>}"
    log "  HTTPS ${tunnel_host} -> ${https_tunnel:-<none>}"
    log "  SVCB ${relay_wildcard} -> ${svcb_relay:-<none>}"
    status=1
  else
    log "PASS: no HTTPS/SVCB/ECH records for the relay wildcard or tunnel host (DNS-only invariant holds)"
  fi
...
case "$MODE" in
  audit-dns)
    ...
    audit_dns "$NAMESPACE" "$TUNNEL_HOST"
    exit $?
    ;;
...
DRY RUN — no changes were made, no secrets were read or printed.
  ...
  frps binary  : ${FRPS_BINARY:-<checksum-verified via ${FRP_FETCH} using ${FRP_MANIFEST}>}
