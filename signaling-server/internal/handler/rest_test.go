...
func TestServeFileNoCache_DisablesCachingForHTMLShell(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filePath, []byte("<!doctype html><title>test</title>"), 0o644))

	req := httptest.NewRequest(http.MethodGet, "/s/test", nil)
	rec := httptest.NewRecorder()
	...
	err := ServeFileNoCache(filePath)(event)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "no-store", rec.Header().Get("Cache-Control"))
}
